...
export const getProjectBoards = async (projectId: string, userId: string) => {
...
export const updateBoard = async (boardId: string, userId: string, data: { name?: string; position?: number }) => {
  const board = await boardRepo.findBoardById(boardId);
  if (!board) throw new AppError('Board not found', 404);
